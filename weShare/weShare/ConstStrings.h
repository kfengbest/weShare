//
//  ConstStrings.h
//  weShare
//
//  Created by fengka on 1/27/13.
//  Copyright (c) 2013 adsk. All rights reserved.
//

#ifndef weShare_ConstStrings_h
#define weShare_ConstStrings_h

static NSString* s_DoubanAPI = @"https://api.douban.com/v2/book/isbn/";

static NSString* s_strApi = @"http://services.sketchbook.cn/openlib/service_test/api.php?";
static NSString* s_strTestApi = @"http://services.sketchbook.cn/test/openshelf/api/index.php?";

static NSString* s_strOp = @"op=";
static NSString* s_strSessionParm = @"&sessionid=";
static NSString* s_strOwnerId = @"&ownerid=";

static NSString* s_GetUserInfoBySession = @"GetUserInfoBySession";
static NSString* s_GetBooksBySession = @"GetBooksBySession";
static NSString* s_GetFriendsBySession = @"GetFriendsBySession";
static NSString* s_GetBooksByOwner = @"GetBooksByOwner";

static NSString* s_loginUrl = @"http://services.sketchbook.cn/openlib/service_test/api.php?op=Login&email=tom.dong@openlib.com&localpwd=c01abe74c44be79ce0bec6f042353064";

#endif
